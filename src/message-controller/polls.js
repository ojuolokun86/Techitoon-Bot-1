...
const createPoll = async (sock, chatId, args, sender) => {
    const [question, ...options] = args.join(' ').split('\n');
    console.log('Creating poll with question:', question); // Log the question
    ...
    const { data, error } = await supabase
        .from('polls')
        .insert({ group_id: chatId, question, options: JSON.stringify(options), votes: JSON.stringify({}), creator: sender, created_at: createdAt });

    if (error) {
        console.error('Error creating poll:', error);
        await sock.sendMessage(chatId, { text: formatResponseWithHeaderFooter('⚠️ Error creating poll.') });
    } else {
        console.log('Poll created successfully:', data); // Log the response data
